 ...
 *
 * THEMING — matches post-card.js: this component supports light/dark mode by
 * reading the `data-theme` attribute on <html> (the same attribute the host
 * page's theme toggle sets). No configuration needed — attach() injects both
 * the dark (default) rules and the `[data-theme="light"] .dac-*` overrides
 * below. If the host page never sets data-theme, the component stays in its
 * original dark look. The light overrides mirror discover.html's palette —
 * white surfaces, soft gray borders, dark text — while accent colors
 * (brand blue, pink) are left untouched since they read fine on both.
 * The Type 3 fullscreen ad is a full-bleed photo with a text scrim, so — like
 * the featured slider in feed.html — its dark-on-image treatment stays fixed
 * across themes; that's image legibility, not page chrome.
 */
(function () {
  'use strict';
  ...
  const CSS = `
    ...
    .dac-native{border-bottom:1px solid #1a1b22;background:#000;font-family:'DM Sans',system-ui,sans-serif;padding:13px 14px 0;transition:background .25s,border-color .25s}
    ...
    /* ── Type 2: Story promo ── */
    .dac-promo{border-bottom:1px solid #1a1b22;background:#000;font-family:'DM Sans',system-ui,sans-serif;padding:13px 14px 14px;cursor:pointer;position:relative;transition:background .25s,border-color .25s}
    ...
    /* ── Type 3: Fullscreen snap ad (image + scrim — unaffected by theme) ── */
    ...
    /* ── Type 4: Banner ── */
    .dac-banner{background:#08090c;border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:11px 13px;display:flex;align-items:center;gap:10px;position:relative;overflow:hidden;font-family:'DM Sans',system-ui,sans-serif;transition:background .25s,border-color .25s}
    ...
    /* ══════════════════════════════════════════════════════════════════
       ...
       <html data-theme="light">. Mirrors post-card.js's token choices
       (white surfaces, soft gray borders, dark text). Accent colors
       (brand blue #38bdf8, accent pink #ff0050/#ff4d7a) are left as-is —
       they read fine on white and stay consistent with the rest of the app.
    ══════════════════════════════════════════════════════════════════ */
    [data-theme="light"] .dac-sp-flag{color:#a3a3a3}

    [data-theme="light"] .dac-native{background:#ffffff;border-bottom-color:#ececec}
    [data-theme="light"] .dac-logo{border-color:rgba(0,0,0,.08)}
    [data-theme="light"] .dac-heading{color:#171717}
    [data-theme="light"] .dac-text{color:#3a3a3a}
    [data-theme="light"] .dac-less{color:#8a8a8a}
    [data-theme="light"] .dac-native-footer{border-top-color:rgba(0,0,0,.06)}
    [data-theme="light"] .dac-act{color:#8a8a8a}
    [data-theme="light"] .dac-act:active{background:rgba(0,0,0,.05)}

    [data-theme="light"] .dac-promo{background:#ffffff;border-bottom-color:#ececec}
    [data-theme="light"] .dac-promo-sp{background:rgba(0,0,0,.05);border-color:rgba(0,0,0,.08);color:#8a8a8a}
    [data-theme="light"] .dac-promo-aname{color:#161616}
    [data-theme="light"] .dac-promo-title{color:#171717}

    [data-theme="light"] .dac-banner{background:#ffffff;border-color:rgba(0,0,0,.08)}
    [data-theme="light"] .dac-banner::after{color:#c2c2c2}
    [data-theme="light"] .dac-banner-logo{border-color:rgba(0,0,0,.08)}
    [data-theme="light"] .dac-banner-headline{color:#171717}
    [data-theme="light"] .dac-banner-sub{color:#8a8a8a}
  `;
