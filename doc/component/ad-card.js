 ...
 *   renderListPlatform(ad) — house ad as a list row (gold fill + CTA)  [Discover]
 *   renderListBook(ad)     — writer-promoted book as a list row (gold border) [Discover]
 */
(function () {
  'use strict';
  ...
  const CSS = `
    ...
    .dac-native{background:#0a0a0c;border-bottom:8px solid #111;padding:14px 14px 12px;cursor:pointer}
    .dac-native-top{display:flex;align-items:center;gap:10px;margin-bottom:10px}
    .dac-logo-mark{width:36px;height:36px;border-radius:10px;flex-shrink:0;background:linear-gradient(135deg,#3b82f6,#6366f1);display:flex;align-items:center;justify-content:center;color:#fff;font-size:13px;font-weight:800}
    ...
    .dac-native-cta{margin-left:auto;background:#3b82f6;color:#fff;border:none;padding:7px 14px;border-radius:10px;font-size:11px;font-weight:800;cursor:pointer;font-family:inherit}

    .dac-platform{margin:0;background:linear-gradient(145deg,#120818 0%,#1a0a14 50%,#0c0c12 100%);border:1px solid rgba(255,0,80,.22);border-radius:16px;overflow:hidden;box-shadow:0 6px 24px rgba(255,0,80,.08);cursor:pointer}
    .dac-plat-top{display:flex;align-items:center;justify-content:space-between;padding:12px 14px 0}
    .dac-plat-badge{display:inline-flex;align-items:center;gap:5px;padding:5px 10px;border-radius:20px;background:rgba(255,0,80,.12);border:1px solid rgba(255,0,80,.2);color:#ff0050;font-size:9px;font-weight:800;letter-spacing:.03em}
    ...
    .dac-plat-icon{width:52px;height:52px;border-radius:14px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:22px;background:linear-gradient(135deg,rgba(255,0,80,.2),rgba(167,139,250,.15));border:1px solid rgba(255,0,80,.25);color:#ff0050}
    ...
    .dac-plat-cta{display:inline-flex;align-items:center;gap:6px;background:#ff0050;color:#fff;border:none;padding:9px 16px;border-radius:12px;font-size:12px;font-weight:800;cursor:pointer;font-family:inherit;box-shadow:0 4px 14px rgba(255,0,80,.28)}
    .dac-plat-foot{padding:8px 14px;border-top:1px solid rgba(255,255,255,.06);font-size:9px;color:#777;font-weight:600}

    .dac-promo{width:100%;background:#0a0a0c;border:1px solid rgba(255,0,80,.18);border-left:3px solid #ff0050;border-radius:15px;overflow:hidden;box-shadow:0 5px 20px rgba(0,0,0,.18);cursor:pointer}
    .dac-promo-top{display:flex;align-items:center;justify-content:space-between;padding:10px 11px 7px}
    .dac-promo-badge{display:inline-flex;align-items:center;gap:5px;padding:5px 9px;border-radius:20px;background:rgba(255,0,80,.07);border:1px solid rgba(255,0,80,.14);color:#ff0050;font-size:8px;font-weight:800;letter-spacing:.02em}
    ...
    .dac-follow{background:#0a0a0c;border:1px solid rgba(167,139,250,.25);border-left:3px solid #a78bfa;border-radius:15px;padding:14px;cursor:pointer;box-shadow:0 5px 18px rgba(0,0,0,.15)}
    .dac-follow-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}
    .dac-follow-badge{display:inline-flex;align-items:center;gap:5px;padding:5px 9px;border-radius:20px;background:rgba(167,139,250,.12);border:1px solid rgba(167,139,250,.22);color:#a78bfa;font-size:8px;font-weight:800;letter-spacing:.02em}
    .dac-follow-sp{font-size:8px;color:#888;font-weight:600}
    .dac-follow-row{display:flex;align-items:center;gap:12px}
    .dac-follow-av{width:56px;height:56px;border-radius:50%;object-fit:cover;flex-shrink:0;border:2px solid rgba(167,139,250,.4);box-shadow:0 0 0 3px rgba(167,139,250,.1)}
    ...
    .dac-follow-cta{margin-top:12px;width:100%;background:#a78bfa;color:#0a0a0c;border:none;padding:10px 14px;border-radius:12px;font-size:12.5px;font-weight:800;cursor:pointer;font-family:inherit;display:flex;align-items:center;justify-content:center;gap:6px}
    ...
    .dac-full-brand{font-size:11px;font-weight:800;color:rgba(255,255,255,.85);margin-bottom:6px}
    ...
    .dac-banner{display:flex;align-items:center;gap:12px;padding:12px 14px;background:#121214;border:1px solid rgba(255,255,255,.07);border-radius:14px;cursor:pointer;margin:4px 0}
    .dac-banner-mark{width:40px;height:40px;border-radius:11px;flex-shrink:0;background:linear-gradient(135deg,#3b82f6,#6366f1);display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;font-weight:800}
    ...
    /* LIST PLATFORM — Discover gold fill */
    .dac-list-platform{display:flex;gap:12px;padding:12px;margin:0;position:relative;border:1.5px solid #c9a227;border-radius:12px;background:#fdf7e6;cursor:pointer}
    .dac-list-platform .dac-lp-cover{width:76px;height:102px;border-radius:9px;overflow:hidden;flex-shrink:0;background:#e8e8ed;box-shadow:0 2px 8px rgba(0,0,0,.08);position:relative;border:1.5px solid #c9a227}
    .dac-list-platform .dac-lp-cover img{width:100%;height:100%;display:block;object-fit:cover}
    .dac-list-platform .dac-lp-tag{position:absolute;top:6px;left:6px;z-index:2;font-size:7px;font-weight:800;letter-spacing:.3px;text-transform:uppercase;padding:2px 6px;border-radius:4px;background:#c9a227;color:#fff}
    .dac-list-platform .dac-lp-body{flex:1;min-width:0;display:flex;flex-direction:column;justify-content:center;gap:4px}
    .dac-list-platform .dac-lp-sponsor{font-size:9px;font-weight:800;color:#c9a227;text-transform:uppercase;letter-spacing:.05em}
    .dac-list-platform .dac-lp-title{font-size:13.5px;font-weight:700;line-height:1.3;color:#1a1a1a;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
    .dac-list-platform .dac-lp-cta{align-self:flex-start;margin-top:4px;font-size:10.5px;font-weight:800;color:#fff;background:#c9a227;padding:5px 12px;border-radius:14px;border:none;font-family:inherit;cursor:pointer}

    /* LIST BOOK — Discover gold border + Promoted */
    .dac-list-book{display:flex;gap:12px;padding:10px;margin:0;position:relative;border:1.5px solid #c9a227;border-radius:12px;background:#fff;cursor:pointer}
    .dac-list-book .dac-lb-cover-link{display:block;flex-shrink:0}
    .dac-list-book .dac-lb-cover{width:76px;height:102px;border-radius:9px;overflow:hidden;background:#e8e8ed;box-shadow:0 2px 8px rgba(0,0,0,.08);position:relative}
    .dac-list-book .dac-lb-cover img{width:100%;height:100%;display:block;object-fit:cover}
    .dac-list-book .dac-lb-tag{position:absolute;top:6px;left:6px;z-index:2;font-size:7px;font-weight:800;letter-spacing:.3px;text-transform:uppercase;padding:2px 6px;border-radius:4px;background:#c9a227;color:#fff}
    .dac-list-book .dac-lb-body{flex:1;min-width:0;display:flex;flex-direction:column;justify-content:center;gap:4px}
    .dac-list-book .dac-lb-genre{font-size:9px;font-weight:800;color:#ff2d55;text-transform:uppercase;letter-spacing:.05em}
    .dac-list-book .dac-lb-title-link{display:block;cursor:pointer}
    .dac-list-book .dac-lb-title{font-size:13.5px;font-weight:700;line-height:1.3;color:#1a1a1a;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
    .dac-list-book .dac-lb-preview{font-size:11.5px;color:#8e8e93;line-height:1.45;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
    .dac-list-book .dac-lb-foot{display:flex;align-items:center;gap:10px;margin-top:2px;flex-wrap:wrap}
    .dac-list-book .dac-lb-stat{display:flex;align-items:center;gap:3px;font-size:10px;color:#8e8e93;font-weight:600}
    ...
    /* Dark theme adjustments for list ads so they still look good on feed */
    [data-theme="dark"] .dac-list-platform{background:#1a1608;border-color:#c9a227}
    [data-theme="dark"] .dac-list-platform .dac-lp-title{color:#f0f0f0}
    [data-theme="dark"] .dac-list-book{background:#121214;border-color:#c9a227}
    [data-theme="dark"] .dac-list-book .dac-lb-title{color:#f0f0f0}
    [data-theme="dark"] .dac-list-book .dac-lb-preview{color:#999}
  `;
  ...
  function renderNative(ad) {
    _injectStyles();
    if ((ad.brand || '').toLowerCase().includes('droboard')) return renderPlatform(ad);
    ...
    return `<div class="dac-native" data-adid="${ad.id}" data-adtype="native">
      <div class="dac-native-top">${logoHtml}
        <div class="dac-brand-wrap"><span class="dac-brand">${_esc(ad.brand)}</span><span class="dac-sp-flag">Sponsored</span></div>
      ...
      <div class="dac-native-footer">
        <div class="dac-act dac-like${ad.liked ? ' dac-liked' : ''}" data-like="${ad.id}"><i class="${ad.liked ? 'fas' : 'far'} fa-heart"></i><span class="dac-like-ct">${_fmtN(ad.likes)}</span></div>
  ...
  function renderPlatform(ad) {
    ...
    return `<div class="dac-platform" data-adid="${ad.id}" data-adtype="platform" data-open="1">
      <div class="dac-plat-top"><div class="dac-plat-badge"><i class="fa-solid fa-bolt"></i> DroBoard</div><span class="dac-plat-sp">Sponsored</span></div>
      <div class="dac-plat-main">
        ...
        <div class="dac-plat-body">
          ...
          <button type="button" class="dac-plat-cta" data-cta="${ad.id}" data-adtype="platform">${_esc(cta)} <i class="fa-solid fa-arrow-right" style="font-size:10px"></i></button>
        </div>
      </div>
  ...
  function renderStoryPromo(ad) {
    ...
    const authorHtml = author ? `<div class="dac-promo-author">${av ? `<img class="dac-promo-av" src="${av}" loading="lazy" alt=""/>` : ''}<span class="dac-promo-aname">${author.startsWith('@') ? _esc(author) : '@' + _esc(author)}</span>${ad.verified !== false ? '<i class="fa-solid fa-circle-check dac-promo-verified"></i>' : ''}</div>` : '';
    return `<div class="dac-promo" data-adid="${ad.id}" data-adtype="storyPromo" data-open="1">
      <div class="dac-promo-top"><div class="dac-promo-badge"><i class="fa-solid fa-wand-magic-sparkles"></i> Sponsored Story</div><div class="dac-promo-sp">Sponsored <button type="button" class="dac-promo-menu" data-ad-menu="${ad.id}" aria-label="More"><i class="fa-solid fa-ellipsis"></i></button></div></div>
      <div class="dac-promo-main">
        <div class="dac-promo-cover-wrap"><img class="dac-promo-cover" src="${cover}" loading="lazy" alt=""/></div>
        <div class="dac-promo-body"><div><div class="dac-promo-cat">${_esc(cat)}</div><div class="dac-promo-title">${_esc(ad.title || '')}</div>${desc ? `<div class="dac-promo-desc">${_esc(desc)}</div>` : ''}${authorHtml}${statsBits.length ? `<div class="dac-promo-stats">${statsBits.join('')}</div>` : ''}</div>
          <div class="dac-promo-action"><button type="button" class="dac-promo-cta" data-cta="${ad.id}" data-adtype="storyPromo"><i class="fa-solid fa-book-open-reader"></i> ${_esc(cta)}</button></div>
        </div>
      </div>
      <div class="dac-promo-bottom"><div class="dac-promo-tags">${tags.slice(0, 3).map(tg => `<span class="dac-promo-tag">${_esc(tg)}</span>`).join('')}</div><span class="dac-promo-promoted">Promoted by author</span></div>
    </div>`;
  }
  ...
  function renderFollowPromo(ad) {
    ...
    return `<div class="dac-follow" data-adid="${ad.id}" data-adtype="follow" data-open="1">
      <div class="dac-follow-top"><div class="dac-follow-badge"><i class="fa-solid fa-user-plus"></i> Suggested Writer</div><span class="dac-follow-sp">Sponsored</span></div>
      <div class="dac-follow-row"><img class="dac-follow-av" src="${av}" loading="lazy" alt=""/><div class="dac-follow-info"><div class="dac-follow-name">${_esc(name)} <i class="fa-solid fa-circle-check" style="color:#38a9ff;font-size:11px"></i></div><div class="dac-follow-handle">@${_esc(handle.replace(/^@/, ''))}</div>${tagline ? `<div class="dac-follow-tagline">${_esc(tagline)}</div>` : ''}</div></div>
      <button type="button" class="dac-follow-cta" data-cta="${ad.id}" data-adtype="follow"><i class="fa-solid fa-user-plus"></i> ${_esc(cta)}</button>
    </div>`;
  }
  ...
  function renderFullscreen(ad) {
    ...
    return `<div class="dac-full" data-adid="${ad.id}" data-adtype="fullscreen" data-open="1">
      <div class="dac-full-bg" style="background-image:url('${ad.bg || ''}')"></div><div class="dac-full-grad"></div>
      ...
      <div class="dac-full-body"><div class="dac-full-brand">${_esc(ad.brand)}</div><div class="dac-full-headline">${_esc(ad.headline)}</div><div class="dac-full-sub">${_esc(ad.sub)}</div>
  ...
  function renderBanner(ad) {
    ...
    return `<div class="dac-banner" data-adid="${ad.id}" data-adtype="banner" data-open="1">
      ...
      <div class="dac-banner-body"><div class="dac-banner-brand">${_esc(ad.brand)}</div><div class="dac-banner-headline">${_esc(ad.headline)}</div><div class="dac-banner-sub">${_esc(ad.sub)}</div></div>
  ...
  function renderListPlatform(ad) {
    ...
    return '<div class="dac-list-platform" data-adid="' + _esc(ad.id || '') + '" data-adtype="listPlatform" data-ad="platform">' +
      '<div class="dac-lp-cover"><span class="dac-lp-tag">Ad</span>' + (img ? '<img src="' + _esc(img) + '" loading="lazy" alt=""/>' : '') + '</div>' +
      '<div class="dac-lp-body"><div class="dac-lp-sponsor">' + _esc(sponsor) + '</div><div class="dac-lp-title">' + _esc(title) + '</div>' +
      '<button type="button" class="dac-lp-cta" data-cta="' + _esc(ad.id || '') + '" data-adtype="listPlatform">' + _esc(cta) + '</button></div></div>';
  }
  ...
  function renderListBook(ad) {
    ...
    return '<div class="dac-list-book" data-adid="' + _esc(ad.id || '') + '" data-adtype="listBook" data-ad="book">' +
      '<a class="dac-lb-cover-link" href="' + _esc(href) + '"><div class="dac-lb-cover"><span class="dac-lb-tag">Promoted</span>' + (img ? '<img src="' + _esc(img) + '" loading="lazy" alt=""/>' : '') + '</div></a>' +
      '<div class="dac-lb-body">' + (genre ? '<div class="dac-lb-genre">' + _esc(genre) + '</div>' : '') +
      '<a class="dac-lb-title-link" href="' + _esc(href) + '"><div class="dac-lb-title">' + _esc(ad.title || '') + '</div></a>' +
      (preview ? '<div class="dac-lb-preview">' + _esc(preview) + '</div>' : '') +
      '<div class="dac-lb-foot">' +
      (author ? '<a class="dac-lb-stat dac-lb-author" href="' + _esc(_hrefs.profileHref(author)) + '" onclick="event.stopPropagation()">' + _esc(author) + '</a>' : '') +
      (ad.rating ? '<div class="dac-lb-stat"><i class="fas fa-star" style="color:#f59e0b"></i> ' + _esc(String(ad.rating)) + '</div>' : '') +
      (ad.chapters ? '<div class="dac-lb-stat"><i class="fas fa-book-open"></i> ' + ad.chapters + ' ch</div>' : '') +
      '</div></div></div>';
  }
    ...
    _root.addEventListener('click', (e) => {
      ...
      if (more) {
        ...
        if (txt) {
          ...
          if (expanded) { txt.classList.add('dac-trunc'); more.className = 'dac-more'; more.innerHTML = 'See more <i class="fas fa-chevron-down" style="font-size:8px"></i>'; }
          else { txt.classList.remove('dac-trunc'); more.className = 'dac-less'; more.innerHTML = 'See less <i class="fas fa-chevron-up" style="font-size:8px"></i>'; }
        }
        return;
      }
      const like = e.target.closest('[data-like]');
      if (like) { e.stopPropagation(); const ad = _findAd(ads, like.dataset.like); if (ad && _hooks.onLike) _hooks.onLike(ad); return; }
      const comment = e.target.closest('[data-comment]');
      if (comment) { e.stopPropagation(); const ad = _findAd(ads, comment.dataset.comment); if (ad && _hooks.onComment) _hooks.onComment(ad); return; }
      const share = e.target.closest('[data-share]');
      if (share) { e.stopPropagation(); const ad = _findAd(ads, share.dataset.share); if (ad && _hooks.onShare) _hooks.onShare(ad, share.dataset.adtype); return; }
      const cta = e.target.closest('[data-cta]');
      if (cta) { e.stopPropagation(); const ad = _findAd(ads, cta.dataset.cta); if (ad && _hooks.onCta) _hooks.onCta(ad, cta.dataset.adtype); return; }
      ...
      if (openable) {
        ...
        if (ad && _hooks.onOpen) _hooks.onOpen(ad, card.dataset.adtype);
      }
    });
  }
  ...
  window.DroboardAdCard = {
    attach, configure,
    renderNative, renderPlatform, renderStoryPromo, renderFollowPromo,
    renderFullscreen, renderBanner, renderListPlatform, renderListBook,
  };
})();
